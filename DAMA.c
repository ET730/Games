...
#define KCYN  "\x1B[36m"
#define KWHT  "\x1B[37m"
void campo(int X[L][L],int D[L+2][L+2]);
...
int main(){
	...
	int X1,Y1,X2,Y2,X,Y,K=0,R,fine=1;
	int D[L+2][L+2]={0},table_2[L+2][L+2];//tavolo dei damoni e tavolo 2 per controlli
	//Inizializzo il campo e il sottocampo con i valori iniziali
	...
	//Inserisco i bordi nel ssottocampo
	for(j=L+1; j>=0; j--, cont++){
		for(i=0; i<L+2; i++){
			if (j==9 || j==0 || i==0 || i==9){
				table_2[i][j]=3;
	...
	campo(table,D); //stampo il campo una prima volta
	while(fine && (V1>0 && V2>0)){ // inizio la partita finchè almeno uno dei giocatori non ha esaurito le pedine
		for (int G=1;G<=2;G++){
			V1=vive(table,1);
			V2=vive(table,2);
			if (V1==0 || V2==0){break;}//se durante i vari turni uno dei due giocatori finisce le pedine termino
			if (V1==1 || V2==1){
				if (V1==1 && G==2){
					if (mangiare(table_2,G,D)){
						fine=0;
						break;
					}
				} else if (V2==1 && G==1){
			...
			do{
				printf("%s",KNRM);
				if (S==0){
					//Se non ho gia mangiato una prima volta in questo turno chiedo coordinate e direzione
				...
				} else if (S==1){ //se ho gia mangiato nello stesso turno e posso mangiare di nuovo chiedo all'utente dove andare
					X=X1;
					Y=Y1;
					do{			//spostamento
						printf("Dove vuoi andare : ");
						scanf("%d%d",&X1,&Y1);
						if (X1==X+1 || X1==X-1){ // se la seconda mossa non è un salto di due rimango fermo
							R=0;
							X1=X;
							Y1=Y;
						} else {
				...
				//se posso mangiare e non ho mangiato gia una volta in questo turno soffio una delle pedine che potevano mangiare
				if (K==1 && S==0){
					for (Y2=L;Y2>0;Y2--){
						for (X2=1;X2<=L;X2++){
							if (table_2[X2][Y2]==G){
								if (table_2[X2+1][Y2+1]!=0 && table_2[X2+1][Y2+1]!=G && table_2[X2+1][Y2+1]!=3 && K==1){
									if (table_2[X2+2][Y2+2]==0){
										table[X2-1][Y2-1]=0;
										table_2[X2][Y2]=0;
										D[X2][Y2]=0;
										K=0;			
								...
								else if (table_2[X2-1][Y2-1]!=0 && table_2[X2-1][Y2-1]!=G && table_2[X2-1][Y2-1]!=3 && K==1){			
									if (table_2[X2-2][Y2-2]==0){
										table[X2-1][Y2-1]=0;
										table_2[X2][Y2]=0;
										D[X2][Y2]=0;
				...
				//se non sono rimasto fermo muovo le pedine
				if (X1==X && Y1==Y){
					K=0;
					S=0;
				} else {
				...
				//se ho mangiato guaardo se posso mangiare di nuovo
				if (S==1){
...
//funziona che controlla se posso mangiare di nuovo
int mangX(int X,int Y,int V[L+2][L+2],int G,int D[L+2][L+2]){
	int K;
	K=damone(D,X,Y);
	if (K==1){ // se nelle cordinate in cui mi sono spostato nel turno posso mangiare restituisco 1 
		if (V[X][Y]==G){  
			if (V[X+1][Y-1]!=0 && V[X+1][Y-1]!=G && V[X+1][Y-1]!=3){
				if (V[X+2][Y-2]==0){
					printf("errore8%d%d",X,Y);
					return 1;
				} else {return 0;}
			} else if (V[X-1][Y+1]!=0 && V[X-1][Y+1]!=G && V[X-1][Y+1]!=3){
				if (V[X-2][Y+2]==0){
					printf("errore7%d%d",X,Y);
					return 1;
				} else {return 0;}
			} else if (V[X-1][Y-1]!=0 && V[X-1][Y-1]!=G && V[X-1][Y-1]!=3){
				if (V[X-2][Y-2]==0){
					printf("errore6%d%d",X,Y);
					return 1;
				} else {return 0;}
			} else if (V[X+1][Y+1]!=0 && V[X+1][Y+1]!=G && V[X+1][Y+1]!=3){
				if (V[X+2][Y+2]==0){
					printf("errore5%d%d",X,Y);
					return 1;
				} else {return 0;}
			}
		}
		...
		if (G==1){
			...
			if(V[X][Y]==G){
				if (V[X-1][Y+1]!=0 && V[X-1][Y+1]!=G && V[X-1][Y+1]!=3 && D[X-1][Y+1]==0){
					if (V[X-2][Y+2]==0){
						printf("errore2%d%d",X,Y);
						return 1;
					} else {return 0;}
				}  else if (V[X+1][Y+1]!=0 && V[X+1][Y+1]!=G && V[X+1][Y+1]!=3 && D[X+1][Y+1]==0){
					if (V[X+2][Y+2]==0){
						printf("errore1%d%d",X,Y);
						return 1;
					} else {return 0;}
				} else {return 0;}
			}
...
int mangiare(int V[L+2][L+2],int G,int D[L+2][L+2]){
	int K; // funzione che controlla se posso mangiare all'inizio del turno
	for (int Y=L;Y>0;Y--){
		for (int X=1;X<L+1;X++){
			K=damone(D,X,Y);
			if (K==1){
...
//funzione che controlla se lo spostamento inserito è valido
int controllo2(int G,int V[L][L],int X,int Y,int X1,int Y1,int D[L+2][L+2]){//modificare il controllo, se è un damone può tornare indietro 
	if (X1>8 || X1<1 || Y1>8 || Y1<1){ // guardo che le coordinate siano valide
		printf("Mossa non valida0\n");
		return 1;
	} else if (damone(D,X,L-Y+1)){ // se la pedina è un damone posso muovermi sia avanti che indietro
	...
	} else { // se non è un damone in base al turno posso andare solo avanti o indietro 
		if (G==1){
...
//funzione che controlla che le coordinate selezionate siano valide e che nel punto ci sia una pedina del giocatore giusto 
int controllo(int G,int V[L][L],int X,int Y){
	if (X>8 || X<1 || Y>8 || Y<1){
		...
		if (Y>1){ // guardo se davanti e dietro di me ho solo pedine di mia proprietà
			if (X>1 && X<8){
...
void campo(int X[L][L],int D[L+2][L+2]){
	...
	char CL[L];
		do{
			...
			for (int i=0;i<L;i++){
				if (i%2==0){
					if ((count/4)%2==0){strcpy(CL,KBLU);}
					else {strcpy(CL,KRED);}
				} else if (i%2==1){
					if ((count/4)%2==0){strcpy(CL,KRED);}
					else {strcpy(CL,KBLU);}
				}
					if(flag==0){
						printf("%s %3d    ",KNRM,i+1);
					}
					if(flag==1){
						printf("%s _____  ",CL);
					}
